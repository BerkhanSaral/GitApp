...
public class UserService {
  ...
  /*password validation: boşluk içermemeli
        ...
        Boolean kucuk = password.contains("[a-z]");
        Boolean buyuk = password.contains("[A-Z]");
        Boolean simge = password.contains(" \\p{Punct}");
        if (password.length() > 6 && !space && (kucuk || buyuk || simge)) {
            isValid = true;
        }
        return isValid;*/
    ...
    /*email validation: boşluk içermemeli
            ...
            int isaret = email.indexOf("@");
            if (isaret == -1 || isaret == email.length() - 1) {
                return false;
            }
            String bolum = String.valueOf(isaret).split("@")[1].trim();
            Boolean son = bolum.contains("[a-zA-Z]");
            Boolean son1 = bolum.contains("[0-9]");
            Boolean son2 = bolum.contains("-");
            Boolean son3 = bolum.contains("_");

            if (!bosluk && (son || son1 || son2 || son3) && (bolum.contains("gmail.com") || bolum.contains("hotmail.com") || bolum.contains("yahoo.com"))) {
                isValid = true;
            }
